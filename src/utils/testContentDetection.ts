...
export const testContentDetection = () => {
  const testCases = [
    {
...
- Imagination!`,
  ...
  testCases.forEach((testCase, index) => {
    ...
    const isPromptOrRequest = (
      ...
      (lowerContent.startsWith('here') && testCase.content.length < 150) ||
      (lowerContent.startsWith('sure') && testCase.content.length < 150) ||
      (lowerContent.startsWith('of course') && testCase.content.length < 150)
    );
    ...
    const hasGeneratedContent = (
      ...
      (testCase.content.split('\n').length > 8 && /\d+\./.test(testCase.content) && testCase.content.length > 300)
    );
    
    const hasSubstantialContent = testCase.content.length > 250;
